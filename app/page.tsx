...
import {
  ...
  Search, ShoppingBag, Sparkles, TicketCheck, TrainFront, Trees,
  UtensilsCrossed, X,
} from 'lucide-react';
...
const categoryNotes: Record<CategoryId, string> = {
  ...
  sights: '랜드마크보다 장면과 동선으로 고른 도시 산책',
  culture: '뮤지컬·미술·재즈를 취향과 체력에 맞게',
  shopping: '백화점, 독립 숍, 마켓에서 고르는 뉴욕의 물건',
    ...
    <article className="detail" aria-labelledby="detail-title">
      ...
      <div className="detail-body">
        <header className="detail-heading">
          <div className="detail-meta"><p className="eyebrow">{item.eyebrow}</p><span>2026. 9. 2. 기준</span></div>
        ...
        <dl className="fact-strip">
          <div><Clock3 /><dt>머무는 시간</dt><dd>{item.duration}</dd></div>
          <div><CircleDollarSign /><dt>예산</dt><dd>{item.budget}</dd></div>
          <div><Sparkles /><dt>좋은 시간</dt><dd>{item.bestTime}</dd></div>
        ...
        <section className="detail-section route-section">
          ...
          <div>
            ...
            <div className="map-frame">
              <iframe src={mapEmbedUrl(item)} title={`${item.name} 주변 지도`} loading="lazy" referrerPolicy="no-referrer" tabIndex={-1} />
...
export default function Home() {
  ...
  const visibleItems = useMemo(() => {
    ...
    return guideItems.filter((item) => item.category === activeCategory && (!showSavedOnly || saved.includes(item.id)) && (!normalized || [item.name, item.englishName, item.area, ...item.tags].join(' ').toLowerCase().includes(normalized)));
  }, [activeCategory, query, saved, showSavedOnly]);
    ...
    <main className="site-shell">
      <header className="masthead">
        ...
        <label className="search-box">
          <Search size={17} /><span className="sr-only">장소 검색</span>
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="장소, 동네, 취향 검색" />
            ...
            {visibleItems.map((item, index) => (
              <article className={`place-row ${selectedItem?.id === item.id ? 'selected' : ''}`} key={item.id}>
                <button className="place-select" type="button" onClick={() => chooseItem(item)} data-place-id={item.id}>
                  ...
                  <span className="place-copy"><span className="place-area">{item.area}</span><strong>{item.name}</strong><span className="place-summary">{item.summary}</span><span className="tag-line">{item.tags.map((tag) => <i key={tag}>{tag}</i>)}</span></span>
                  <ChevronRight className="row-arrow" size={20} />
                </button>
                <button className={`save-button ${saved.includes(item.id) ? 'saved' : ''}`} type="button" onClick={() => toggleSaved(item.id)} aria-label={`${item.name} ${saved.includes(item.id) ? '저장 취소' : '저장'}`}><Bookmark size={17} fill={saved.includes(item.id) ? 'currentColor' : 'none'} /></button>
              </article>
            ))}
            {!visibleItems.length && <div className="empty-state"><Glasses /><p>{showSavedOnly ? '아직 저장한 장소가 없어요.' : '이 카테고리에서 검색 결과를 찾지 못했어요.'}</p>{showSavedOnly && <button type="button" onClick={() => setShowSavedOnly(false)}>전체 목록 보기</button>}</div>}
          </div>
        </div>
