...
const STORAGE_KEY = "nyc-couple-trip-v2-03";
...
const BOOKING_STEPS = [
  ["국제선 + 무료 취소 호텔", "9/18 저녁 직항과 9/26 낮 직항을 먼저 묶고, UN 본부 동쪽은 피합니다."],
  ["Broadway 9/23 수 19:00", "Hamilton이 일정상 가장 안정적입니다. 공식 티켓을 우선하고 TKTS를 예비로 둡니다."],
  ["나이아가라 국내선·1박", "플랜 03 선택 시 9/21–22 또는 9/22–23 왕복. 귀국편과 최소 이틀의 완충 시간을 둡니다."],
  ["자유의 여신상 페리·전망대", "자유의 여신상은 오전 공식 페리, 전망대는 일몰 60–75분 전 입장 시간으로 예약하는 것이 좋습니다."],
...
const FIELD_NOTES = [
  ...
  ["OMNY는 각자 하나의 결제 수단", "기본요금은 $3이며, 같은 카드나 기기를 사용하면 7일 동안 $35의 요금 상한이 자동 적용됩니다."],
  ["JFK에는 출국 3시간 전 도착", "낮 직항이면 07:30 전후 호텔 출발을 기준으로 잡습니다. AirTrain은 편도 $8.75입니다."],
...
function handleRawImageError(event) {
  const image = event.currentTarget;
  if (image.dataset.fallback === "used") { image.hidden = true; return; }
  image.dataset.fallback = "used";
  image.src = tripData.images.hero;
}
...
function DayRail({ plan, dayIndex, arrival, completed, onChange }) {
  ...
  const onKeyDown = (event, index) => {
    ...
    if (["ArrowRight", "ArrowDown"].includes(event.key)) next = (index + 1) % plan.days.length;
    if (["ArrowLeft", "ArrowUp"].includes(event.key)) next = (index - 1 + plan.days.length) % plan.days.length;
  ...
  return (
    <nav className="day-rail chapter-rail" aria-label="7일 여행 날짜">
      <header className="chapter-heading"><div><strong>여정 챕터</strong><span>7일 전체 흐름</span></div><small>PLAN {plan.number}</small></header>
      <div className="day-tabs" role="tablist" aria-label="날짜별 일정" aria-orientation="vertical">
        {plan.days.map((day, index) => {
          ...
          const booking = day.stops.find((stop) => stop.reserve)?.reserve;
          return <button id={`day-tab-${index}`} key={day.date} className="day-tab" type="button" role="tab" aria-selected={index === dayIndex} tabIndex={index === dayIndex ? 0 : -1} onClick={() => onChange(index)} onKeyDown={(event) => onKeyDown(event, index)}><span className="day-medallion"><b>D{index + 1}</b><small>{parts.dateAndWeekday.split(" ")[0]}</small></span><span className="day-rail-copy"><small>{parts.dateAndWeekday}</small><strong>{day.title}</strong><em>{booking || day.subtitle}</em><span><PersonSimpleWalk size={13} />{day.walk}{count > 0 && ` · ${count}/${day.stops.length} 완료`}</span></span><img src={day.photo} alt="" loading="lazy" decoding="async" onError={handleRawImageError} /></button>;
        })}
      </div>
      <footer className="chapter-footer"><span><MapTrifold size={16} />하루 한 권역</span><span><Bed size={16} />휴식 우선</span></footer>
    </nav>
  );
}
...
function Itinerary({ day, dayIndex, selectedIndex, expandedIndices, favorites, completed, advice, itemRefs, onSelect, onToggleFavorite, onToggleComplete, onCopy }) {
  const selected = day.stops[selectedIndex] || day.stops[0];
  const next = day.stops[selectedIndex + 1] || null;
  const nextSchedule = next;
  const moveDuration = next?.move?.match(/\d+분/)?.[0] || "여유";
  return (
    <section className="itinerary-panel field-itinerary" aria-labelledby="day-heading">
      <header className="progress-heading"><div><h1 id="day-heading">오늘의 진행</h1><span>{day.date.split("·")[0].trim()}</span></div><small>{day.weather}</small></header>
      <section className="progress-summary" aria-label="선택한 일정과 다음 일정"><div><small>선택 일정</small><strong>{selected?.time || "—"}</strong></div><div className="is-priority"><small>다음 일정</small><strong>{nextSchedule?.time || "마무리"}</strong></div><div><small>이동</small><strong>{moveDuration}</strong></div></section>
          ...
          return (
            <li key={`${stop.time}-${stop.title}`} ref={(node) => { itemRefs.current[index] = node; }} className={`itinerary-stop ${isSelected ? "is-selected" : ""} ${isComplete ? "is-complete" : ""}`} data-tone={meta.tone}>
              <button className="stop-select" type="button" aria-expanded={isExpanded} aria-current={isSelected ? "step" : undefined} onClick={() => onSelect(index, true)}>
                <span className="stop-time">{stop.time}<small>{stop.end}</small></span><KindIcon className="stop-kind-icon" size={18} /><span className="stop-marker">{index + 1}</span>
                <span className="stop-copy"><span className="stop-meta">{meta.label}{duration && ` · ${duration}`}</span><strong>{stop.title}</strong><small>{stop.detail}</small></span><span className="stop-thumb"><img src={media.src} alt="" loading="lazy" decoding="async" onError={handleRawImageError} /></span>
      ...
      <footer className="day-total"><span><NavigationArrow size={17} /> {day.stops.length}개 장소</span><span><PersonSimpleWalk size={17} /> {day.walk}</span><span><Bed size={17} /> {day.rest}</span></footer>
      <details className="swap-note"><summary><CloudRain size={17} /> 날씨·일정 교체 팁</summary><p>{day.swap}</p></details>
    </section>
  );
}

function ContextPanel({ plan, day, selectedIndex, favorites, completed, note, onNoteChange, onSelect, onToggleFavorite, onToggleComplete, onCopy }) {
  const stop = day.stops[selectedIndex] || day.stops[0]; const media = getMedia(stop.title, tripData, day.photo, day.credit);
  const relatedStops = [...day.stops.slice(selectedIndex + 1), ...day.stops.slice(0, selectedIndex)];
  const secondaryMedia = relatedStops.map((item) => getMedia(item.title, tripData, day.photo, day.credit)).find((item) => item.src !== media.src) || { src: tripData.images.hero, credit: tripData.credits.hero };
  const meta = getKindMeta(stop.kind); const KindIcon = KIND_ICONS[stop.kind] || Circle;
  const photoCaption = media.key === "hotelRoom" ? "뉴욕 호텔 객실 · 분위기 참고" : stop.title;
  return (
    <aside className="context-panel map-stage" aria-label="지도와 선택 장소 상세">
      <div className="map-canvas-wrap"><div className="map-legend" aria-label="지도 범례"><span className="is-active"><MapTrifold size={17} />지도</span><span><Train size={17} />지하철</span><span><PersonSimpleWalk size={17} />도보</span><span><Check size={17} />완료</span></div><RouteMap day={day} selectedIndex={selectedIndex} onSelect={onSelect} completed={completed} /><aside className="regional-inset" aria-label="도심과 근교 연결"><img src={plan.thumbnail} alt="" onError={handleRawImageError} /><div><small>도심 + 근교</small><strong>{plan.nature}</strong><span>선택한 플랜의 자연 구간</span></div></aside><div className="map-tools"><span>사진 핀과 일정 번호가 같습니다.</span><a href={getGoogleRouteUrl(day)} target="_blank" rel="noreferrer">Google Maps <ArrowSquareOut size={14} /></a></div></div>
      ...
      <article className="place-dossier place-sheet"><p className="sr-only" role="status" aria-live="polite">{stop.time} {stop.title} 선택됨</p><div className="sheet-handle" aria-hidden="true" /><div className="place-media-pair"><PhotoFigure src={media.src} fallback={day.photo} fallbackCredit={day.credit} alt={media.key === "hotelRoom" ? "뉴욕 호텔 객실 분위기 참고 사진" : `${stop.title} 분위기`} credit={media.credit} caption={photoCaption} className="place-photo" priority /><PhotoFigure src={secondaryMedia.src} fallback={tripData.images.hero} fallbackCredit={tripData.credits.hero} alt={`${stop.title} 다음 동선의 분위기`} credit={secondaryMedia.credit} caption="다음 장면" className="place-photo-secondary" /></div><div className="sheet-copy"><div className="place-kicker"><span>{selectedIndex + 1}</span><KindIcon size={16} />{meta.label}</div><div className="place-heading"><div><h3>{stop.title}</h3><p>{stop.time}{stop.end && `–${stop.end}`} · {stop.move}</p></div><div className="sheet-quick-actions"><IconAction label={favorites.includes(selectedIndex) ? "즐겨찾기 해제" : "즐겨찾기"} aria-pressed={favorites.includes(selectedIndex)} onClick={() => onToggleFavorite(selectedIndex)}><Heart size={20} weight={favorites.includes(selectedIndex) ? "fill" : "regular"} /></IconAction></div></div><dl className="place-facts"><div><dt>왜 이곳인가</dt><dd>{stop.detail}</dd></div><div><dt>머무는 시간</dt><dd>{formatDuration(getDurationMinutes(stop.time, stop.end)) || "시간 여유"}</dd></div><div><dt>예약</dt><dd>{stop.reserve || "현장 진행"}</dd></div><div><dt>다음 이동</dt><dd>{nextStopMove(day, selectedIndex)}</dd></div></dl><TravelNote key={`${plan.id}-${day.date}`} note={note} onNoteChange={onNoteChange} /></div><div className="place-actions"><a className="primary-action" href={getPlaceMapUrl(stop)} target="_blank" rel="noreferrer"><NavigationArrow size={18} /> 길찾기</a><button type="button" onClick={() => onCopy(stop)}><Copy size={18} /> 예약 정보 복사</button>{stop.url && <a href={stop.url} target="_blank" rel="noreferrer"><ArrowSquareOut size={18} /> 공식 정보</a>}<button type="button" aria-pressed={completed.includes(selectedIndex)} onClick={() => onToggleComplete(selectedIndex)}>{completed.includes(selectedIndex) ? <Check size={19} weight="bold" /> : <Circle size={19} />} 완료</button></div></article>
    </aside>
  );
}

function AtlasStrip({ onOpen }) {
  return <section className="atlas-strip field-atlas" aria-labelledby="atlas-strip-title"><header><h2 id="atlas-strip-title">장소 아틀라스</h2><p>지도로 담는 뉴욕<br />문화 · 맛 · 자연</p><button type="button" onClick={() => onOpen("places")}>모두 보기 <ArrowRight size={17} /></button></header><div className="atlas-cards">{ATLAS_ITEMS.map((item, index) => <button key={item.key} className="atlas-card" type="button" onClick={() => onOpen(["pizza", "lobster", "porterhouse"].includes(item.key) ? "food" : "places")}><span className="atlas-category">{index < 2 ? "문화" : index < 5 ? "맛" : "자연"}</span><img src={tripData.images[item.key]} alt={`${item.label} 분위기`} loading="lazy" decoding="async" onError={handleRawImageError} /><span><strong>{item.label}</strong><small>{item.sub}</small></span></button>)}</div></section>;
}
...
function AtlasOverlay({ plan, activeTab, onTab, onClose, onJumpToDay, onPrint }) {
  ...
  const tabs = [["overview", "여행 개요"], ["places", "장소"], ["food", "맛"], ["stays", "숙소"], ["booking", "예약"], ["sources", "출처"]];
...
export function App() {
  ...
  return (
    <div className={`app-shell mobile-mode-${mobileView}`}>
      ...
      <header className="app-header field-header"><div className="brand-lockup"><span>NYC</span><strong>둘이서 천천히</strong></div><span className="header-divider" /><div className="trip-dates"><CalendarBlank size={17} />9/18–9/27 <small>(10일)</small></div><div className="city-route"><AirplaneTilt size={16} /> ICN → JFK → 테네시 → 산호세</div>
        ...
        <div className="header-menu-wrap"><button className="header-plan" type="button" aria-haspopup="true" aria-expanded={planMenuOpen} onClick={() => { setPlanMenuOpen((open) => !open); setArrivalOpen(false); }}><span>{plan.number}</span>{plan.short}<CaretDown size={14} /></button>{planMenuOpen && <div className="plan-menu" aria-label="여행 플랜 선택">{tripData.plans.map((item, index) => <button key={item.id} type="button" aria-pressed={index === planIndex} onClick={() => changePlan(index)}><img src={item.thumbnail} alt="" onError={handleRawImageError} /><span><small>PLAN {item.number} · 강도 {item.pace}</small><strong>{item.short}</strong><em>{item.bestFor} · {item.nature}</em></span>{index === planIndex && <Check size={18} />}</button>)}</div>}</div><div className="active-day-label">D{dayIndex + 1} · {dateParts(day.date).dateAndWeekday}</div><div className="header-spacer" />
      ...
      {city !== "nyc" ? <CityEmpty city={city} onBack={() => setCity("nyc")} /> : <><section className="mobile-operational" aria-label="선택한 일정과 다음 일정"><div><small>선택 일정</small><strong>{selectedStop.time}</strong></div><div><small>다음 일정</small><strong>{nextStop?.time || "마무리"}</strong></div><div><small>이동</small><strong>{nextStop?.move || "오늘 일정 마무리"}</strong></div></section>
        <main id="primary-content" tabIndex={-1} className={`primary-grid field-guide-layout mobile-view-${mobileView}`}><DayRail plan={plan} dayIndex={dayIndex} arrival={arrival} completed={completed} onChange={changeDay} /><ContextPanel plan={plan} day={day} selectedIndex={activeIndex} favorites={favoriteList} completed={completedList} note={note} onNoteChange={(value) => setNotes((current) => ({ ...current, [dayKey]: value }))} onSelect={(index) => selectStop(index, false, true)} onToggleFavorite={(index) => toggleIndex(setFavorites, index)} onToggleComplete={(index) => toggleIndex(setCompleted, index)} onCopy={copyStop} /><Itinerary day={day} dayIndex={dayIndex} selectedIndex={activeIndex} expandedIndices={expandedIndices} favorites={favoriteList} completed={completedList} advice={getArrivalAdvice(arrival, dayIndex)} itemRefs={itemRefs} onSelect={selectStop} onToggleFavorite={(index) => toggleIndex(setFavorites, index)} onToggleComplete={(index) => toggleIndex(setCompleted, index)} onCopy={copyStop} /></main><AtlasStrip onOpen={openAtlas} />
