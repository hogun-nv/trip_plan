...
const STORAGE_KEY = "nyc-couple-trip-v2-02";
...
const ATLAS_ITEMS = [
  ...
  { key: "moma", label: "MoMA", sub: "현대미술 · Midtown" },
  { key: "whitney", label: "Whitney", sub: "미국 미술 · Chelsea" },
  { key: "pizza", label: "뉴욕 피자", sub: "접어 먹는 한 조각" },
  { key: "bagel", label: "베이글과 록스", sub: "뉴욕식 아침" },
  { key: "lobster", label: "해산물", sub: "대서양의 맛" },
  { key: "porterhouse", label: "클래식 스테이크", sub: "둘이 나누는 저녁" },
  { key: "central", label: "Central Park", sub: "공원 · 산책" },
  { key: "highLine", label: "High Line", sub: "도시 위의 정원" },
  { key: "dumbo", label: "브루클린", sub: "물가에서 보는 맨해튼" },
  { key: "times", label: "브로드웨이", sub: "오늘의 무대, 내일의 감동" },
  { key: "hero", label: "East River", sub: "해질녘의 스카이라인" },
];
...
function getInitialState() {
  ...
  const savedDayIndex = Number.isInteger(saved.dayIndex) ? saved.dayIndex : 3;
  return {
    ...
    selectedIndex: Math.max(0, Number.isInteger(saved.selectedIndex) ? saved.selectedIndex : 1),
    ...
    mobileView: ["photos", "itinerary", "map"].includes(saved.mobileView) ? saved.mobileView : "itinerary",
  };
}
...
function CinematicRail({ planIndex, city, onPlan, onCity, onOpen }) {
  return (
    <aside className="cinematic-rail" aria-label="여행과 플랜 전환">
      <header><strong>둘이서<br />천천히</strong><span>9/18–9/27</span></header>
      <nav className="rail-cities" aria-label="방문 도시">
        {[["nyc", "뉴욕"], ["tennessee", "테네시"], ["sanjose", "산호세"]].map(([id, label]) => <button type="button" key={id} aria-current={city === id ? "page" : undefined} onClick={() => onCity(id)}>{label}</button>)}
      </nav>
      <section className="rail-chapters" aria-labelledby="chapter-title">
        <p id="chapter-title">여정 챕터</p>
        {tripData.plans.map((plan, index) => <button type="button" key={plan.id} aria-current={index === planIndex ? "step" : undefined} onClick={() => onPlan(index)}><span>{plan.number}</span><strong>{plan.short}</strong><small>{plan.bestFor}</small></button>)}
      </section>
      <nav className="rail-tools" aria-label="여행 도구">
        <button type="button" onClick={() => onOpen("overview")}><List size={16} /> 여행 개요</button>
        <button type="button" onClick={() => onOpen("places")}><MapTrifold size={16} /> 장소 아틀라스</button>
        <button type="button" onClick={() => onOpen("food")}><Heart size={16} /> 맛 아틀라스</button>
        <button type="button" onClick={() => onOpen("booking")}><Ticket size={16} /> 예약 관리</button>
      </nav>
    </aside>
  );
}
...
function TravelNote({ day, dayIndex, note, onNoteChange }) {
  const [open, setOpen] = useState(Boolean(note));
  return (
    <section className="travel-note">
      <div className="cover-kicker">D{dayIndex + 1} · {dateParts(day.date).dateAndWeekday}</div>
      <h2 className="cover-title">{day.title}</h2>
      <p className="cover-summary">{day.subtitle}</p>
      <div className="cover-facts" aria-label="하루 요약"><span><PersonSimpleWalk size={14} />{day.walk}</span><span><Bed size={14} />{day.rest}</span></div>
...
function EditorialGallery({ plan, day, dayIndex, note, onNoteChange }) {
  ...
  const dayLead = getMedia(day.title, tripData, day.photo, day.credit);
  const main = { src: dayLead.src, credit: dayLead.credit, title: day.title };
  ...
  return (
    <aside className="editorial-gallery" aria-label="선택한 날의 분위기">
      ...
      <TravelNote key={`${plan.id}-${day.date}`} day={day} dayIndex={dayIndex} note={note} onNoteChange={onNoteChange} />
    </aside>
  );
}
          ...
          return (
            <li key={`${stop.time}-${stop.title}`} ref={(node) => { itemRefs.current[index] = node; }} className={`itinerary-stop ${isSelected ? "is-selected" : ""} ${isComplete ? "is-complete" : ""}`} data-tone={meta.tone}>
              <button className="stop-select" type="button" aria-expanded={isExpanded} aria-current={isSelected ? "step" : undefined} onClick={() => onSelect(index, true)}>
                <span className="stop-time">{stop.time}<small>{stop.end}</small></span><span className="stop-marker">{index + 1}</span><span className="stop-thumb"><img src={media.src} alt="" loading="lazy" decoding="async" onError={handleRawImageError} /></span>
...
function ContextPanel({ plan, day, selectedIndex, favorites, completed, onSelect, onToggleFavorite, onToggleComplete, onCopy }) {
  ...
  const detailKey = /Keens|Gallaghers|Peter Luger|steak/i.test(stop.title) ? "porterhouse" : /Oyster|Fulton|Lobster|seafood/i.test(stop.title) ? "lobster" : /Pizza|Juliana|Joe's|John's|Scarr/i.test(stop.title) ? "pizza" : /bagel|Zabar|Russ/i.test(stop.title) ? "bagel" : null;
  const contextualStop = day.stops.find((candidate, index) => {
    if (index === selectedIndex || candidate.kind === "FOOD") return false;
    return getMedia(candidate.title, tripData, day.photo, day.credit).src !== media.src;
  });
  const contextualMedia = contextualStop ? getMedia(contextualStop.title, tripData, day.photo, day.credit) : { src: day.photo, credit: day.credit };
  const detailMedia = detailKey ? { src: tripData.images[detailKey], credit: tripData.credits[detailKey], caption: "대표 메뉴" } : { src: contextualMedia.src, credit: contextualMedia.credit, caption: contextualStop?.title || "오늘의 장면" };
  return (
    <aside className="context-panel" aria-label="지도와 선택 장소 상세">
      ...
      <article className="place-dossier"><p className="sr-only" role="status" aria-live="polite">{stop.time} {stop.title} 선택됨</p><div className="place-kicker"><span>{selectedIndex + 1}</span><KindIcon size={16} />{meta.label}</div><div className="place-heading"><div><h3>{stop.title}</h3><p>{stop.time}{stop.end && `–${stop.end}`} · {stop.move}</p></div><span className="place-reserve">{stop.reserve || "예약 없음"}</span></div><div className="dossier-gallery"><PhotoFigure src={media.src} fallback={day.photo} fallbackCredit={day.credit} alt={media.key === "hotelRoom" ? "뉴욕 호텔 객실 분위기 참고 사진" : `${stop.title} 공간 분위기`} credit={media.credit} caption={photoCaption} className="place-photo" /><PhotoFigure src={detailMedia.src} fallback={tripData.images.hero} fallbackCredit={tripData.credits.hero} alt={`${stop.title}와 함께 보는 뉴욕의 디테일`} credit={detailMedia.credit} caption={detailMedia.caption} className="place-photo" /></div><h4>왜 여기인가</h4><p>{stop.detail}</p><div className="place-stats"><span><Clock size={16} />{formatDuration(getDurationMinutes(stop.time, stop.end)) || "시간 여유"}</span><span><Ticket size={16} />{stop.reserve || "현장 진행"}</span>{stop.url && <a className="official-link" href={stop.url} target="_blank" rel="noreferrer">공식 정보 <ArrowSquareOut size={14} /></a>}</div><div className="place-actions"><a className="primary-action" href={getPlaceMapUrl(stop)} target="_blank" rel="noreferrer"><NavigationArrow size={18} /> 길찾기</a><IconAction label="예약 정보 복사" onClick={() => onCopy(stop)}><Copy size={20} /></IconAction><IconAction label={favorites.includes(selectedIndex) ? "즐겨찾기 해제" : "즐겨찾기"} aria-pressed={favorites.includes(selectedIndex)} onClick={() => onToggleFavorite(selectedIndex)}><Heart size={20} weight={favorites.includes(selectedIndex) ? "fill" : "regular"} /></IconAction><IconAction label={completed.includes(selectedIndex) ? "완료 취소" : "완료 표시"} aria-pressed={completed.includes(selectedIndex)} onClick={() => onToggleComplete(selectedIndex)}>{completed.includes(selectedIndex) ? <Check size={20} /> : <Circle size={20} />}</IconAction></div></article>
    </aside>
  );
}

function AtlasStrip({ onOpen }) {
  return <section className="atlas-strip" aria-labelledby="atlas-strip-title"><header><h2 id="atlas-strip-title">뉴욕의 장면들</h2><button type="button" onClick={() => onOpen("places")}>모두 보기 <ArrowRight size={17} /></button></header><div className="atlas-cards">{ATLAS_ITEMS.map((item) => <button key={item.key} className="atlas-card" type="button" onClick={() => onOpen(["pizza", "bagel", "lobster", "porterhouse"].includes(item.key) ? "food" : "places")}><img src={tripData.images[item.key]} alt={`${item.label} 분위기`} loading="lazy" decoding="async" onError={handleRawImageError} /><span><strong>{item.label}</strong><small>{item.sub}</small></span></button>)}</div></section>;
}
...
export function App() {
  ...
  const switchMobileView = (view) => { setMobileView(view); if (view === "itinerary") { window.requestAnimationFrame(() => scrollToStop(activeIndex)); } else { window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: "auto" })); } };
  ...
  return (
    <div className={`app-shell mobile-mode-${mobileView}`}>
      ...
      <CinematicRail planIndex={planIndex} city={city} onPlan={changePlan} onCity={setCity} onOpen={openAtlas} />
      ...
      {city !== "nyc" ? <CityEmpty city={city} onBack={() => setCity("nyc")} /> : <><DayRail plan={plan} dayIndex={dayIndex} arrival={arrival} completed={completed} onChange={changeDay} /><section className="mobile-operational" aria-label="선택한 일정과 다음 일정"><div><small>선택 일정</small><strong>{selectedStop.time}</strong></div><div><small>다음 일정</small><strong>{nextStop?.time || "마무리"}</strong></div><div><small>이동</small><strong>{nextStop?.move || "오늘 일정 마무리"}</strong></div></section>
        ...
        <nav className="mobile-bottom-nav" aria-label="현장용 주요 화면"><button type="button" aria-current={mobileView === "photos" ? "page" : undefined} onClick={() => switchMobileView("photos")}><Images size={21} />사진</button><button type="button" aria-current={mobileView === "itinerary" ? "page" : undefined} onClick={() => switchMobileView("itinerary")}><List size={21} />일정</button><button type="button" aria-current={mobileView === "map" ? "page" : undefined} onClick={() => switchMobileView("map")}><MapTrifold size={21} />지도</button><button type="button" onClick={() => openAtlas("places")}><Binoculars size={21} />아틀라스</button></nav></>}
